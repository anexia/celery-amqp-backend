...
class AMQPBackend(base.BaseBackend):
    ...
    def __init__(self, app, exchange=None, exchange_type=None, persistent=None, serializer=None, auto_delete=True,
                 **kwargs):
        ...
        self.result_exchange_type = exchange_type or conf.result_exchange_type or 'direct'
    ...
    def store_result(self, task_id, result, state, traceback=None, request=None, **kwargs):
        ...
        routing_key, correlation_id = self._create_routing_key(task_id), request and request.correlation_id or task_id
    ...
    def wait_for(self, task_id, timeout=None, cache=True, no_ack=True, on_message=None, on_interval=None, **kwargs):
        """
    ...
    def get_many(self, task_ids, timeout=None, no_ack=True, cache=True, on_message=None, on_interval=None, **kwargs):
        ...
        if cache:
            for task_id in task_ids:
                ...
                if cached_task_result and cached_task_result['status'] in self.READY_STATES:
        ...
        with self.app.pool.acquire_channel(block=True) as (conn, channel):
            ...
            def on_message_callback(message):
                ...
                received_task_state, received_task_id = received_task_result['status'], received_task_result['task_id']
            ...
            with self.Consumer(channel, bindings, on_message=on_message_callback, accept=self.accept, no_ack=no_ack):
...
class DirectReplyAMQPBackend(base.BaseBackend):
    ...
    def get_many(self, task_ids, timeout=None, interval=0.5, on_interval=None, no_ack=True, **kwargs):
    ...
    def get_task_meta(self, task_id, backlog_limit=1000):
        ...
        for _ in range(backlog_limit):
            ...
            except (KeyError, amqp.exceptions.UnexpectedFrame):
                time.sleep(0)
            except socket.timeout:
                break
            else:
